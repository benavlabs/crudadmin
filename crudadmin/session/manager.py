...
class SessionManager:
    ...
    async def create_session(
        ...
        try:
            ...
            async for admin_session in self.db_config.get_admin_db():
                try:
                    existing_sessions = await self.get_user_active_sessions(admin_session, user_id)
                    if len(existing_sessions) >= self.max_sessions:
                        logger.info(f"Max sessions ({self.max_sessions}) reached, deactivating old sessions")
                        for session in existing_sessions:
                            await self.terminate_session(admin_session, session["session_id"])
                            await admin_session.commit()

                    logger.info("Creating new session in database")
                    result = await self.db_config.crud_sessions.create(admin_session, object=session_data)
                    ...
                    await admin_session.commit()
                    logger.info(f"Session {session_id} created and committed successfully")
                    return session_data
    ...
    async def validate_session(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error processing session last_activity: {str(e)}", exc_info=True)
                return False
    ...
    async def get_session_metadata(self, db, session_id: str) -> Optional[dict]:
        ...
        session = await self.db_config.crud_sessions.get(
            db,
            session_id=session_id
        )
        if not session:
